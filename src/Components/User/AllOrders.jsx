            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} transition={{ duration: 0.5 }}>
                <div className="bg-[var(--bg-secondary)] rounded-2xl p-8 backdrop-blur-sm border border-[var(--border-primary)]">
                    <div className="flex flex-col sm:flex-row justify-start items-center mb-5 pb-3 gap-5 border-b border-[var(--border-primary)]">
                        ...
                        <h1 className='text-md text-center font-bold text-[var(--text-primary)] uppercase'>
                    ...
                    <div className="space-y-4">
                        {orders.map((order) => {
                            const maxHeight = orders.length * 50;
                            return <div key={order._id} className="flex flex-col md:flex-row justify-between bg-[var(--bg-primary)] p-3 md:p-6 rounded-xl border border-[var(--border-secondary)] hover:border-[var(--border-primary)] transition-all cursor-pointer"
                                onClick={() => setOrderDetails(orderDetails === order._id ? null : order._id)}>
                                <div className='transition-all duration-500 flex-1 overflow-hidden'>
                                    <h2 className="text-md font-semibold text-[var(--text-primary)]">Order ID: {order._id}</h2>
                                    <p className="text-[var(--text-secondary)]">Ordered on: {formatDate(order.createdAt)}</p>
                                    <p className="text-[var(--text-secondary)]">Total Price: {order.totalOrderPrice} EGP</p>
                                    ...
                                    <div className={`transition-all duration-500 overflow-hidden ${orderDetails === order._id ? `max-h-[600px] opacity-100 mt-4` : 'max-h-0 opacity-0'}`}>
                                        <p className="text-[var(--text-secondary)]">Payment Method: {order.paymentMethodType}</p>
                                        ...
                                        <div className="mt-4 space-y-4 max-h-[400px] overflow-y-auto">
                                            {order.cartItems.map((item, index) => (
                                                <div key={index} className="flex flex-col sm:flex-row items-center sm:items-start space-x-0 sm:space-x-4 p-3 rounded-lg bg-[var(--bg-secondary)]">

                                                    <img src={item.product.imageCover} alt={item.product.title} className="w-12 h-16 object-fit rounded-lg border border-gray-200 dark:border-gray-500" />

                                                    <div className="flex-1 text-center sm:text-left mt-3 sm:mt-0">
                                                        <h3 className="text-md font-medium text-gray-800 dark:text-gray-200 line-clamp-3">{item.product.title}</h3>
                                                        <div className='flex flex-wrap justify-center sm:justify-start gap-3 items-center mt-2'>
                                                            ...
                                                            <span className='hidden sm:block border-r border-gray-300 dark:border-gray-500 h-5'></span>
                                ...
                                <button className="hidden md:block ps-2 rounded-full transition-colors"
